...
#include "battery_monitor/InputAccepted.h"
...
void BatteryCallback(const sensor_msgs::BatteryState::ConstPtr& msg)
{
  ...
  if ((safety_threshold != -1) && (mission_threshold != -1)) { //the thresholds must be set for the status to be published
    ...
    if ((current_battery > mission_threshold) && (current_battery <= kMaxBatteryLevel)){
      output_msg.status = battery_monitor::BatteryStatus::OK;
    } else if ((current_battery <= mission_threshold) && (current_battery > safety_threshold )) {
      output_msg.status = battery_monitor::BatteryStatus::MISSION_CRITICAL;
    } else if ((current_battery <= safety_threshold) && (current_battery >= kMinBatteryLevel)) {
      output_msg.status = battery_monitor::BatteryStatus::SAFETY_CRITICAL;
    } else {
      ROS_ERROR("Invalid battery reading.");
      output_msg.status = battery_monitor::BatteryStatus::BATTERY_READING_OUT_OF_SCOPE;
    }
  }
...
int main(int argc, char **argv)
{
  ...
  input_status_pub = nh.advertise<battery_monitor::InputAccepted>("input_accepted",1000);
